...
class ChatController extends Controller
{
    ...
    public function store(Request $request, User $user)
    {
        ...
        $message = Message::create(['sender_id'=>$auth->id,'receiver_id'=>$user->id,'message'=>$data['message']]);
        Notification::create(['user_id'=>$user->id,'title'=>'Chat baru','message'=>$auth->name.' mengirim pesan baru.','url'=>route('chats.show',$auth)]);
        try { Mail::raw($auth->name." mengirim pesan chat baru:\n\n".$data['message'], fn($mail)=>$mail->to($user->email)->subject('Chat baru SIM Skripsi')); } catch (\Throwable $e) {}
        if ($request->expectsJson()) return response()->json(['ok'=>true,'id'=>$message->id]);
        return redirect()->route('chats.show', $user)->with('success','Pesan berhasil dikirim.');
    }
